...
export function Seo({ title, description = siteConfig.description, path = '/', image, jsonLd = [] }) {
  const fullTitle = title ? `${title} - ${siteConfig.name}` : `${siteConfig.name} - Interior Design and Turnkey Execution`
...
export function localBusinessSchema() {
  return {
    ...
    address: {
      '@type': 'PostalAddress',
      streetAddress: 'GD-87, GD Block, Dakshini Pitampura, Pitampura',
