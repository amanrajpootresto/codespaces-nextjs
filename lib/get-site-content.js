...
async function fetchSiteContent() {
  ...
  try {
    ...
    return normalizeRevalidateSeconds(mergeContent(fallbackContent, adaptStarterSheet(result.content)))
  } catch (error) {
    console.error('Unable to load remote website content; using local fallback:', error)
    return normalizeRevalidateSeconds(fallbackContent)
  }
}
...
export async function getSiteContent() {
  ...
  pendingRequest = fetchSiteContent().then((content) => {
    ...
    const revalidateSeconds = Number(content.contentApi?.revalidateSeconds) || 60
    cacheExpiresAt = Date.now() + revalidateSeconds * 1000
    return content
...
export function contentPageProps(content) {
  const revalidateSeconds = Number(content.contentApi?.revalidateSeconds) || 60
  return { props: { siteContent: content }, revalidate: revalidateSeconds }
}
